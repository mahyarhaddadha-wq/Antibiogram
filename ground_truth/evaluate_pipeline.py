...
اجرا:
    python3 ground_truth/evaluate_pipeline.py [--kernel antibiogram-test]
خروجی: گزارشِ متنیِ کامل در stdout + ذخیره‌ی جزئیاتِ خام در
    ground_truth/evaluation_results.csv
"""
import argparse
import copy
import csv
...
def run_pipeline(image_path: Path, kernel_name: str):
...
def main():
    ...
    for img in images:
        ...
        print(f"=== {img} ===")
        sys_dishes = run_pipeline(image_path, args.kernel)
        ...
        for gi in unmatched_gt:
            ...
            all_rows.append({"image": img, "gt_disk_number": g["disk_number"],
                              "match_dist_px": None, "gt_halo_mm": g["halo_diameter_mm_expert"],
        ...
        for si in unmatched_sys:
            ...
            all_rows.append({"image": img, "gt_disk_number": None,
                              "match_dist_px": None, "gt_halo_mm": None,
